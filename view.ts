...
async onOpen() {
     //const container = this.containerEl.children[1];//Creacion del container
     const container = document.createElement("div");
     const container1 = document.createElement("div");
     
    container.className = "my-container";
     container1.className = "searchBox";
// Agrega el contenedor a la vista de Markdown
     this.containerEl.children[1].appendChild(container1);
...
//Caja de busqueda
 ...
 container1.appendChild(searchBox);
...
async function buscar(valor: string) {
       const response = query().then((dato) => {
         ...
         const resultados = dato.filter(item => item.toLowerCase().includes(valor.toLowerCase()));
